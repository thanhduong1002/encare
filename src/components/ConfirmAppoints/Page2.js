...
const Page2 = () => {
    const [appcon, setAppCon] = useState([]);
  const navigate = useNavigate();
  let token = localStorage.getItem('token');
  useEffect(() => {
    axios({
      baseURL:
        'https://encare-doctor.herokuapp.com/api/doctor/appointments/status=2?page=2',
      method: 'get',
      headers: {
